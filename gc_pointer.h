...
public:
  ...
  Iter<T> begin() {
    unsigned _size = (isArray) ? arraySize : 1;
...
// Copy constructor.
template<class T, int size>
Pointer<T, size>::Pointer(const Pointer &ob) : addr(ob.addr),
...
void Pointer<T, size>::showlist() {
  ...
  for (p = refContainer.begin(); p != refContainer.end(); ++p) {
    ...
    if (p->memPtr) std::cout << " " << *p->memPtr << std::endl;
    else           std::cout << "---\n";
  }
  std::cout << std::endl;
}
